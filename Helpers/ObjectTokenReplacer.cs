...
{
    public class ObjectTokenReplacer
    {
        
        public ObjectTokenReplacer()
        {
            Initialize();
        }
        public ObjectTokenReplacer(object obj)
        {
            Initialize(); 
            AddObject(obj);
        }
        public ObjectTokenReplacer(bool loadUserInfo)
        {
            Initialize();
            if (loadUserInfo)
            {
        ...
        public string ReplaceFirstOccurenceOnly(string original)
        {
            var newValue = original;

            //Check for DateTime token
            var regex = Regex.Match(newValue, $@"({{[L,U]?{{)(DateTime[:,-,\s]?)(.*)(}}}})");

            if (regex.Success)
            {
                var replace = regex.Groups[0];
                var pattern = regex.Groups[3].Value;
                var formattedDate = DateTime.Now.ToString(pattern);
                ...
                ReplacementDictionary[regex.Groups[0].Value] = formattedDate;
            }
            if (Fields == null || Fields.Count < 1)
            {
                return ReplacementDictionary.Aggregate(newValue, (current, item) => current.Replace(item.Key, item.Value));
            }
            //Check for existing field token
            foreach (var field in Fields)
            {
                regex = Regex.Match(newValue, $@"({{[L,U]?{{)(\[\d+\])?({field.FieldName})(\[\d+\])?({{.}})?({{\d}})?(\[\d+\])?(}}}})");
                if (regex.Success)
                {                                                                                             
                    // value contains token for this field. 
                    var upper = false;
                    ...
                    else
                    {
                        if (multiValueCheck.Success && regex.Groups[4].Value != "")
                        {
                            ...
                            ReplacementDictionary[regex.Groups[0].Value] = strValue;
                        }
                        else
                        {
                            // Single Value Field
